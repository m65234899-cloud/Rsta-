...
- الإحترام أولاً وآخراً وقبل كل شيء سواء للاعضاء او للادارة او العليا

- يمنع السب أو المزح الثقيل بالشات حتى لو شخص تمون عليه

- يمنع استخدام الصلاحيات خارج الإدارة حتى لو مزح

- عدم تكبير المواضيع بين اثنين يمزحون مزح خفيف

- يمنع الدق بالكلام أو خلق مشاكل

- عدم مجادلة الإداري حتى لو كان غلطان وفتح تكت للمشكلة

- ممنوع التحذير بالشات

- ممنوع مجادلة العليا في أي قرار
...
if (content === "!") {
  ...
  const names = role.members.map(m => `<@${m.id}>`).join("\n");

  const embed = new EmbedBuilder()
    .setTitle("📌 المصممين")
    .setDescription(names || "لا يوجد مصممين حالياً")
    .setColor(0x00ffff);
